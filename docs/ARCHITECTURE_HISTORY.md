...
### Architecture Cleanup — Parallel Track: Runtime Visual Token Sync (in progress)
...
**Solution so far**: The active Snake game runtime now comes from `js/game/*` through `app.js` instead of the older bundled `js/snake-game.js` entry. `js/game/config.js` now derives its live palette from computed CSS tokens, `ThemeController` exposes explicit subscriptions for theme changes, and `app.js` wires theme updates to `SnakeGame.syncTheme()` without synthetic cross-module DOM events. `js/guess-track.js` now derives burst palettes from theme tokens, `js/waveformRenderer.js` reads semantic token values for its gradients and glow layers at draw time, and the remaining runtime fallback literals have been centralized in `js/ui/runtimeColorFallbacks.js` so they no longer drift independently.
