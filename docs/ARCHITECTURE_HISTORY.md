...
**Solution so far**: Extracted YouTubePlayerAdapter, AssistantClient, SyncExporter, PointWorkspaceRenderer, PointTimingEngine, ReviewPlayerPanel, StudioWorkflowState, and StudioEventBindings from studio; extracted PreviewAudioEngine and WaveformRenderer from features. Public APIs established in Phase 1 stayed unchanged.

**Current state**: `features.js` is down to 763 LOC and `timeSyncStudio.js` is down to 1,127 LOC. The studio now keeps orchestration while `pointTimingEngine.js` owns autosync and mutation state, `reviewPlayerPanel.js` owns review-player panel state and loop-tick rendering decisions, `studioWorkflowState.js` owns the setup/loading/empty/lyrics/export workflow presets, and `studioEventBindings.js` owns DOM listener binding and cleanup. The seams are materially better, but the studio file still has more decomposition work ahead if it grows further.
