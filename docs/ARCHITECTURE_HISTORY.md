...
### Architecture Cleanup — Parallel Track: CSS Token Migration (completed)
...
**Solution**: Added shared overlay, outline, scrim, input-shadow, and surface-shadow tokens to `css/base.css`. `conversion-animations.css` moved off its remaining direct color literals earlier in the cleanup, `features.css` plus `time-sync-page.css` now consume the shared tokens for their repeated glass-surface treatments, `karaoke-panel.css` now uses the same token set instead of component-level overlay and warning literals, `guess-track.css` now routes its state surfaces and Frutiger overrides through token-backed custom properties instead of raw component literals, `game.css` now does the same for state chrome, canvas shadows, and Frutiger glass overrides, `form.css` now consumes semantic input inset-shadow tokens, and the final header/theme-switcher/preview shading literals have been moved onto header-scoped or semantic tokens.
...
### Architecture Cleanup — Parallel Track: Runtime Visual Token Sync (completed)

**Problem**: Even after the component CSS pass, some game and preview visuals still hardcoded colors inside JavaScript and canvas renderers, which meant theme changes did not consistently flow through semantic tokens.

**Solution**: The active Snake game runtime now comes from `js/game/*` through `app.js` instead of the older bundled `js/snake-game.js` entry. `js/game/config.js` now derives its live palette from computed CSS tokens, `ThemeController` exposes explicit subscriptions for theme changes, and `app.js` wires theme updates to `SnakeGame.syncTheme()` without synthetic cross-module DOM events. `js/guess-track.js` now derives burst palettes from theme tokens, `js/waveformRenderer.js` reads semantic token values for its gradients and glow layers at draw time, and the remaining runtime fallback literals are centralized in `js/ui/runtimeColorFallbacks.js` as documented CSS-unavailable defaults.

**Key design choice**: Move live rendering onto semantic tokens first, but keep a narrow layer of hardcoded fallback values inside the renderers until a second shared runtime palette abstraction is justified. This keeps the theme system authoritative without over-centralizing one-off canvas defaults.
