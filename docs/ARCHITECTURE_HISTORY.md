...
**Solution so far**: Extracted YouTubePlayerAdapter, AssistantClient, SyncExporter, PointWorkspaceRenderer, PointTimingEngine, and ReviewPlayerPanel from studio; extracted PreviewAudioEngine and WaveformRenderer from features. Public APIs established in Phase 1 stayed unchanged.

**Current state**: `features.js` is down to 763 LOC and `timeSyncStudio.js` is down to 1,341 LOC. The studio now keeps orchestration and DOM wiring while `pointTimingEngine.js` owns autosync and mutation state, and `reviewPlayerPanel.js` owns review-player panel state and loop-tick rendering decisions. The seams are materially better, but the studio file still has more decomposition work ahead if it grows further.
